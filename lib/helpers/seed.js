...
module.exports = async({ 
  ...
  const studios = await Studio.create([...Array(studio)].map(() => ({
    name: `${chance.prefix()} ${chance.gender()} ${chance.animal()}`
  })));
  ...
  await Review.create([...Array(review)].map(() => ({
    ...
    review: `${chance.pickone(actors.map(actor => actor.name))} was ${chance.pickone(['fantastic', 'terrible', 'just okay', 'mediocre', 'a hot mess'])}. It was the ${chance.pickone(['best', 'worst', 'blandest', 'sleepiest', 'funniest'])} ${chance.weekday()} of my life.`,
